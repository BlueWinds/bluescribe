...
const FileContents = ({ filename }) => {
  ...
  return <details open={true} className="edit-file">
    ...
    <div className="grid columns">
      <div className="selections">
        <select onChange={e => setEntryType(e.target.value)} defaultValue={entryType}>
          {Object.entries(file.type === 'gameSystem' ? gameSystemTypes : catalogueTypes).map(([type, name]) => <option key={type} value={type}>{name}</option>)}
        </select>
        <table><tbody>
