...
export const Checkbox = ({ entry, field, label, updateFile, defaultValue = false, ...props }) => <tr {...props}>
  ...
  <td><input type="checkbox" checked={entry[field] ?? defaultValue } name={field} onChange={e => {
...
export const Name = ({ entry, updateFile, children, ...props }) => <tr {...props}>
  <td colSpan="2">
    {children}
    <h6>
      <input value={entry.name || ''} onChange={e => { entry.name = e.target.value; updateFile()}} />
    </h6>
  </td>
</tr>
Name.propTypes = {
...
export const Id = ({ entry, updateFile, ...props }) => <tr {...props}>
  <td>Id</td>
  <td>
    <button className="refresh-id outline" onClick={() => { entry.id = randomId(); updateFile()}} data-tooltip-id="tooltip" data-tooltip-html="Generate new id">⟳</button>
...
export const Profiles = ({ filename, entry, updateFile, ...props }) => {
  ...
  return <>
    <tr {...props} >
      <th colSpan="2">
        <button className="add-entry outline" disabled={!(system.profileTypes?.length > 0)} onClick={() => {
          ...
          updateFile()
        }} data-tooltip-id="tooltip" data-tooltip-html="Add profile">+</button>
        Profiles
      </th>
    </tr>
    {entry.profiles?.map(profile => <tr key={profile.id} {...props} >
    ...
    <tr {...props}>
      <th colSpan="2">
        <button className="add-entry outline" onClick={() => {
          ...
          updateFile()
        }} data-tooltip-id="tooltip" data-tooltip-html="Add modifier group">±</button>
        <button className="add-entry outline" onClick={() => {
          ...
          updateFile()
        }} data-tooltip-id="tooltip" data-tooltip-html="Add modifier">+</button>
        Modifiers
      </th>
    </tr>
    {on.modifiers?.map((modifier, i) => <tr key={i} {...props}>
      <td colSpan="2"><Modifier entry={entry} on={on} modifier={modifier} filename={filename} /></td>
    </tr>)}
    {on.modifierGroups?.map((modifierGroup, i) => <tr key={'group-' + i} {...props}>
      <td colSpan="2"><ModifierGroup entry={entry} on={on} modifierGroup={modifierGroup} filename={filename} /></td>
    </tr>)}
  </>
}
Modifiers.propTypes = {
