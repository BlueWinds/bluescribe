...
const Repeat = ({ entry, filename, modifier }) => {
  ...
  files.forEach(file => scopeOptions.push(...Object.values(file.ids).filter(x => (
    x.__type === 'selectionEntry'
    || x.__type === 'selectionEntryGroup'
    || x.__type === 'entryLink'
  ))))

  const childIdOptions = []
  files.forEach(file => childIdOptions.push(...Object.values(file.ids).filter(x => (
    x.__type === 'selectionEntry'
    || x.__type === 'selectionEntryGroup'
    || x.__type === 'category'
  ))))
  ...
  return <>
    ...
    <tr data-indent="2">
      ...
      <td>
        <select defaultValue={repeat.field} onChange={e => { repeat.field = e.target.value; updateFile() }}>
          <option value="selections">Selections</option>
          <option value="forces">Forces</option>
          {_.flatten(files.map(f => f.costTypes?.map(ct => <option key={ct.typeId} value={ct.typeId}>{ct.name}</option>)))}
        </select>
      </td>
    </tr>

    <tr data-indent="2">
      <td><label htmlFor="childId">Scope</label></td>
      <td>
        <ReferenceSelect
          name="childId"
          isClearable={false}
    ...
    <Checkbox entry={repeat} field="shared" label="Shared" updateFile={updateFile} defaultValue={true} data-indent="2" />
    <Checkbox entry={repeat} field="includeChildForces" label="Include child Forces" updateFile={updateFile} data-indent="2" />
    <Checkbox entry={repeat} field="includeChildSelections" label="Include child Selections" updateFile={updateFile} data-indent="2" />

    <tr data-indent="2" >
      <td><label htmlFor="repeats">Repeat</label></td>
      <td><input type="number" value={repeat.repeats} name="repeats" onChange={e => { repeat.repeats = e.target.value; updateFile() }} /></td>
    </tr>
