...
const Modifier = ({ entry, on, filename, modifier }) => {
  ...
  gatherFiles(file, gameData).forEach(f => f.costTypes?.forEach(ct => fields[ct.name] = {label: ct.name, value: 'number', type: ['set', 'increment', 'decrement']}))

  entry.constraints?.forEach(c => fields[c.id] = {label: `Constraint: ${c.scope} ${c.field} ${c.type} ${c.value}`, value: 'number', type: ['set', 'increment', 'decrement']})
  ...
  return <details>
    ...
    <table><tbody>
      ...
      <tr>
        ...
        <td><select defaultValue={modifier.type} name="type" onChange={e => {
          ...
          if (e.target.value !== 'increment' && e.target.value !== 'decrement') { delete modifier.repeats }
      ...
      <tr>
        ...
        <td>
          <select name="field" defaultValue={modifier.field} onChange={e => {
            ...
            {Object.keys(fields).map(field => fields[field].type.includes(modifier.type) && <option key={field} value={field}>{fields[field].label}</option>)}
          </select>
        </td>
      </tr>
      ...
      {(modifier.type === 'increment' || modifier.type === 'decrement') && <Repeats entry={entry} modifier={modifier} updateFile={updateFile} filename={filename} />}
    </tbody></table>
  </details>
}
