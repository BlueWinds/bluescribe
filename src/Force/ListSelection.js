...
const ListSelection = ({ indent, path, selection, selectedPath, setSelectedPath }) => {
  ...
  const upgrades = (selection.selections?.selection.filter((s) => !s.selections) || [])
    .map(selectionName)
    .sort()
    .join(', ')
  ...
  return (
    <>
      ...
      {_.sortBy(selection.selections?.selection, 'name')
        .filter(
          (s, i) =>
            s.selections || rosterErrors[`${path}.selections.selection.${selection.selections.selection.indexOf(s)}`],
        )
        .map((subSelection) => (
          <ListSelection
            key={subSelection.id}
