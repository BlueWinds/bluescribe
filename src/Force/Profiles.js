...
const Profiles = ({ profiles, number }) => {
  return (
    <>
      {_.sortBy(Object.keys(profiles), (p) => order[p] || p).map((type) => (
        <table className="profile" key={type}>
          <thead>
            <tr>
              <th>{type}</th>
              {profiles[type][0][1].characteristics.characteristic.map((c) => (
                <th key={c.name}>{c.name}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {_.sortBy(profiles[type], '1.name').map(([number, profile]) => (
              <tr key={profile.id}>
                <td>
                  {number > 1 ? `x${number} ` : ''}
                  {profile.name}
